...
A five-minute tune-up for any Claude Code setup. One command walks you through four opt-in upgrades — your existing `CLAUDE.md` is backed up before anything is touched.
...
4. **claude-hud status line** — installs [jarrodwatts/claude-hud](https://github.com/jarrodwatts/claude-hud), a heads-up status line showing model, context usage, and session info.
...
Pick any combination of the four items. Everything is reversible — the setup command reports every change it makes and how to undo it.
...
hooks/hooks.json                SessionStart lessons nudge (silent unless needed)
scripts/lessons-check.cjs        the nudge script (no LLM, ~50ms, never blocks startup)
